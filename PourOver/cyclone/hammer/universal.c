...
static void universal_anything(t_universal *x, t_symbol *s, int ac, t_atom *av)
{
	...
	{
	    if (ac > 1)
		universal_dolist(x->x_glist, (int)x->x_descend, s, ac, av);
	    else
		universal_dofloat(x->x_glist, (int)x->x_descend, s, av->a_w.w_float);
	}
	else if (av->a_type == A_SYMBOL)
	{
	    if (av->a_w.w_symbol == &s_bang)
		universal_dobang(x->x_glist, (int)x->x_descend, s);
	    else if (av->a_w.w_symbol == &s_float)
	    {
		if (ac == 1)
		    universal_dofloat(x->x_glist, (int)x->x_descend, s, 0.);
		else if (av[1].a_type == A_FLOAT)
		    universal_dofloat(x->x_glist, (int)x->x_descend, s,
				      av[1].a_w.w_float);
		else
		    loud_error((t_pd *)x, "Bad argument for message 'float'");
	    }
	    else if (av->a_w.w_symbol == &s_symbol)
		universal_dosymbol(x->x_glist, (int)x->x_descend, s,
				   (ac > 1 && av[1].a_type == A_SYMBOL ?
				    av[1].a_w.w_symbol : &s_));
	    else if (av->a_w.w_symbol == &s_list)
		universal_dolist(x->x_glist, (int)x->x_descend, s, ac - 1, av + 1);
	    else
		universal_doanything(x->x_glist, (int)x->x_descend, s,
				     av->a_w.w_symbol, ac - 1, av + 1);
	}
	if (av->a_type == A_POINTER)
	    universal_dopointer(x->x_glist, (int)x->x_descend, s,
				av->a_w.w_gpointer);
    }
}
