...
{
    int range = (int)port_getint(x, 8),
	left = (int)port_getint(x, 3),
	top = (int)port_getint(x, 4),
	right = (int)port_getint(x, 5),
	bottom = (int)port_getint(x, 6),
	flags = (int)port_getint(x, 7);
    import_emstart(x, portps_vtable, port_getsymbol(x, 9), (int)port_getint(x, 2));
#ifdef PORT_DEBUG
    loudbug_post(
...
static int imaction_K1_replace(t_port *x, char *arg)
{
    ...
    {
	char buf[32];
	x->x_emsize = (int)port_getint(x, 2);
	x->x_emcount = 0;
...
static int imaction_K1_set(t_port *x, char *arg)
{
    ...
    {
	int i, count = (int)port_getint(x, 2);
	if (count != x->x_emcount)
	    loud_warning(0, "import",
