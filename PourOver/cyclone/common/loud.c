		      ...
		      t_symbol *s, int ac, t_atom *av)
{
    ...
    if (callername)
    {
	lc->lc_cnsize = (int)strlen(callername) + 1;
	...
	lc->lc_andindent = (int)strlen(class_getname(*caller)) + 10;
    else if (s)
	lc->lc_andindent = (int)strlen(s->s_name) + 10;
    else
	lc->lc_andindent = (int)strlen(LOUD_ERROR_DEFAULT) + 1;
}
